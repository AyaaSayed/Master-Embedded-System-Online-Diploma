...
void HAL_LCD_Send_Command( uint8_t command )
{
	...
	_delay_ms(1);                                       // delay for processing Tas = 50ns
		...
		MCAL_GPIO_WritePort(LCD_DATA_PORT, ((MCAL_GPIO_ReadPort(LCD_DATA_PORT) & 0xFF0F) | ((command & 0x0F) << 4)));
...
void HAL_LCD_Display_Character( uint8_t data )
{
   ...
   _delay_ms(1);                                        // delay for processing Tas = 50ns
		...
		MCAL_GPIO_WritePort(LCD_DATA_PORT, ((MCAL_GPIO_ReadPort(LCD_DATA_PORT) & 0xFF0F) | (data & 0xF0)));
		...
		MCAL_GPIO_WritePort(LCD_DATA_PORT, ((MCAL_GPIO_ReadPort(LCD_DATA_PORT) & 0xFF0F) | ((data & 0x0F) << 4)));
...
void HAL_LCD_display_String( const char *Str )
{
	uint16_t count = 0;
